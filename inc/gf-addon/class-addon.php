...
class GF_Recurly extends GFPaymentAddOn {
			...
			array(
				...
				'fields'      => array(
					...
					array(
						...
						'choices' => array(
							array(
								'label' => esc_html__( 'Subscription', 'gravityformsrecurly' ),
								'value' => 'subscription'
							),
						),
	...
	public function billing_info_fields() {
		...
		$fields = array(
			array(
				'name'     => 'firstName',
				'label'    => esc_html__( 'First Name', 'gravityformsrecurly' ),
				'required' => true
			),
			array(
				'name'     => 'lastName',
				'label'    => esc_html__( 'Last Name', 'gravityformsrecurly' ),
				'required' => true
			),
			array(
				'name'     => 'username',
				'label'    => esc_html__( 'Username', 'gravityformsrecurly' ),
				'required' => false
			),
			...
			array(
				'name'     => 'company',
				'label'    => esc_html__( 'Company', 'gravityformsrecurly' ),
				'required' => false
			),
			array( 'name' => 'address', 'label' => esc_html__( 'Address', 'gravityformsrecurly' ), 'required' => true ),
			array(
				'name'     => 'address2',
				'label'    => esc_html__( 'Address 2', 'gravityformsrecurly' ),
				'required' => false
			),
			...
			array(
				'name'     => 'recurlyCoupon',
				'label'    => esc_html__( 'Recurly Coupon', 'gravityformsrecurly' ),
				'required' => false
			)
		);
	...
	public function plugin_settings_fields() {
		...
		return array(
			...
			array(
				'title'       => esc_html__( 'Webhooks', 'gravityformsrecurly' ),
				'description' => __( "Endpoint URL: " ) . '<code>' . $this->get_endpoint_url() . '</code>',
				'fields'      => array()
			)
		);
	}
	...
	public function supported_notification_events( $form ) {
		...
		return array(
			...
			'create_recurly_account'    => esc_html__( 'Recurly Account Created', 'gravityformsrecurly' )
		);
	...
	public function get_endpoint_url() {
		...
		return ( defined( 'GF_RECURLY_ENDPOINT_BASE' ) ? add_query_arg( 'callback', $this->_slug, GF_RECURLY_ENDPOINT_BASE ) : get_bloginfo( 'url' ) . '/?callback=' . $this->_slug );
	}

	/**
	 * Callback action for new Recurly account webhook
	 *
	 * @since
	 * @access public
	 *
	 ...
	 * @return bool
	 */
	public function create_recurly_account( $entry, $action ) {
	...
	/**
	 * Get data for callback processing
	 *
	 * @since
	 * @access public
	 *
	...
	public function callback() {
		...
		switch ( $notification->type ) {
			...
			case 'new_account_notification':
				...
				//If new account, assumes it only has one subscription since it was just created

				try {

					$subscriptions = Recurly_SubscriptionList::getForAccount( $notification->account->account_code ); //This does not work. Implementing workaround

					$subscriptions = $subscriptions->_get( $subscriptions->getHref() );

					/**
					 * @var Recurly_SubscriptionList $subscriptions
					 */

					if ( ! empty( $subscriptions ) && is_object( $subscriptions ) && 1 == $subscriptions->count() ) {

						$subscription = $subscriptions->current();

					} else {

						$this->log_error( __METHOD__ . "(): Account either has none or more than 1 subscription" );

						return new WP_Error( 'subscription_not_found', sprintf( __( 'Account id: %s either has none or more than one subscription. Webhook cannot be processed.', 'gravityformsrecurly' ), $notification->account->account_code ) );

					}

				} catch ( Recurly_NotFoundError $e ) {

					$this->log_error( __METHOD__ . "(): Not Found: {$e->getMessage()}" );

					return new WP_Error( 'account_not_found', sprintf( __( 'Account id: %s was not found. Webhook cannot be processed.', 'gravityformsrecurly' ), $notification->account->account_code ) );
				...
				$subscription_id = $subscription->__get( 'uuid' );

				$entry_id = $this->get_entry_by_transaction_id( $subscription_id );

				if ( ! $entry_id ) {

					return new WP_Error( 'entry_not_found', sprintf( __( 'Entry for transaction id: %s was not found. Webhook cannot be processed.', 'gravityformsrecurly' ), $subscription_id ) );
				 ...
				 * - account exists, and
				 * - attached subscription was created the same day as entry, because a subscription can't exist before account
				 *
				 * Since webhooks can be delivered at any time in any order, if subscription has since been canceled, do nothing
				 */

				if ( strstr( $subscription->__get( 'activated_at' )->date, ' ', true ) == strstr( $entry[ 'date_created' ], ' ', true ) && 'active' == $subscription->__get( 'state' ) ) {

					$callback_action = array(
						'type'     => 'create_recurly_account',
						'callback' => 'create_recurly_account',
						'id'       => "newacct_{$notification->account->account_code}",
						'entry_id' => $entry_id
					);
			...
			case 'new_subscription_notification':
				...
				$subscription_id = $notification->subscription->uuid[ 0 ];
				 ...
				 * - still active
				 * - was created the same day as entry
				 *
				 * Since webhooks can be delivered at any time in any order, if subscription is no longer active, do nothing
				 */
				...
				try {
					...
					return new WP_Error( 'subscription_not_found', sprintf( __( 'Subscription id: %s was not found. Webhook cannot be processed.', 'gravityformsrecurly' ), $subscription_id ) );
				...
				if ( strstr( $subscription->__get( 'activated_at' )->date, ' ', true ) == strstr( $entry[ 'date_created' ], ' ', true ) && 'active' == $subscription->__get( 'state' ) ) {
					...
					$callback_action = array(
						...
						'entry_id'        => $entry_id,
						'id'              => "newsubs_{$subscription_id}",
						'subscription_id' => (string) $subscription_id,
						'amount'          => $recurly->cents_to_dollars( $notification->subscription->total_amount_in_cents * $notification->subscription->quantity )
					);
			...
			case 'successful_payment_notification':
				...
				$subscription_id = $notification->transaction->subscription_id;
				 ...
				 * - it's for same subscription_id, and
				 * - status is success
				 *
				 * No matter when webhook was delivered, a transaction's status and info shouldn't change
				 */

				try {

					$transaction = Recurly_Transaction::get( $notification->transaction->id );

				} catch ( Recurly_NotFoundError $e ) {

					$this->log_error( __METHOD__ . "(): Not Found: {$e->getMessage()}" );

					return new WP_Error( 'transaction_not_found', sprintf( __( 'Transaction id: %s was not found. Webhook cannot be processed.', 'gravityformsrecurly' ), $notification->transaction->id ) );


				}

				if ( 'success' == $transaction->__get( 'status' ) && false !== strpos( $transaction->__get( 'subscription' )->getHref(), (string) $notification->transaction->subscription_id ) ) {


					$callback_action = array(
						'type'            => 'add_subscription_payment',
						'entry_id'        => $entry_id,
						'id'              => "paysuccess_{$notification->transaction->id}",
						'transaction_id'  => (string) $notification->transaction->id,
						'subscription_id' => (string) $subscription_id,
						'amount'          => $recurly->cents_to_dollars( $notification->transaction->amount_in_cents )
					);
			...
			case 'failed_payment_notification':
				...
				if ( 'Declined' == $transaction->__get( 'status' ) && false !== strpos( $transaction->__get( 'subscription' )->getHref(), (string) $notification->transaction->subscription_id ) ) {

					$callback_action = array(
						'type'            => 'fail_subscription_payment',
						'entry_id'        => $entry_id,
						'id'              => "payfail_{$notification->transaction->id}",
						'transaction_id'  => (string) $notification->transaction->id,
						'subscription_id' => (string) $subscription_id,
						'amount'          => $recurly->cents_to_dollars( $notification->transaction->amount_in_cents )
					);
